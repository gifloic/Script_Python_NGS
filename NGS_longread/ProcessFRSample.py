...
class Process:
    ...
    def run_process(self):
        ...
        process_path = '{}_{}_{}'.format(self.input_name,self.primer_index,self.condition)
        if not os.path.isdir(process_path):
            os.mkdir(process_path)

        os.chdir('ini_files/')

        f1 = '{}_{}_{}_1'.format(self.input_name,self.primer_index,self.condition)
        f2 = '{}_{}_{}_2'.format(self.input_name,self.primer_index,self.condition)
        for fq in [f1,f2]:
        ...
        cmd1 = '{}/ReadParser.py ' \
               '-f {}.fastq -o {}.out -i {} -e forward {} {} --mpi'.format( self.exe_dir,
                                                                        f1, f1, self.primer_index, flag_barcode,
                                                                        flag_config)
        cmd2 = '{}/ReadParser.py ' \
               '-f {}.fastq -o {}.out -i 1 -e reverse -r {} --mpi'.format(self.exe_dir,
                                                                          f2, f2, flag_config)
...
def Main():
    ...
    parser.add_argument('-p', '--primer_index', action="store", help='Index of the primer used for PCR in the forward strand')
